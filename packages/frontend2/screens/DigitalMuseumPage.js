...
export default function DigitalMuseumPage({ navigation, route }) {
  ...
  // If wallId exists, filter pieces client-side to only show pieces for that wall
  // This runs whenever pieces change (e.g. after useData fetches them)
  useEffect(() => {
    if (initialWallId.current) {
      const filtered = pieces.filter(
        (p) => p.wall?._id === initialWallId.current,
      );
  ...
  // Filter logic — runs when search query, active filters or pieces change
  useEffect(() => {
    // Skip filter logic if we're in wall filter mode
    if (initialWallId.current) return;
    ...
    if (normalizedQuery) {
      results = results.filter((piece) => {
        const year = piece.date
          ? new Date(piece.date).getFullYear().toString()
          : "";
        const month = piece.date
          ? new Date(piece.date)
    ...
    if (activeFilters.yearMin !== null) {
      results = results.filter((piece) => {
        ...
        return (
          pieceYear >= activeFilters.yearMin &&
          pieceYear <= (activeFilters.yearMax ?? activeFilters.yearMin)
        );
      });
    }
    ...
    if (activeFilters.months.length > 0) {
      results = results.filter(
        (piece) =>
          piece.date &&
          activeFilters.months.includes(new Date(piece.date).getMonth()),
      );
    }

    if (activeFilters.styles.length > 0) {
      results = results.filter((piece) =>
        activeFilters.styles.includes(
          piece.graffitiStyle?.name ?? piece.graffitiStyle?.graffitiStyleName,
        ),
      );
    }

    if (activeFilters.artistSort === "az") {
      results.sort((pieceA, pieceB) =>
        (pieceA.user?.username || "").localeCompare(
          pieceB.user?.username || "",
        ),
      );
    } else if (activeFilters.artistSort === "za") {
      results.sort((pieceA, pieceB) =>
        (pieceB.user?.username || "").localeCompare(
          pieceA.user?.username || "",
        ),
      );
    }
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        <View style={styles.center}>
          <Text style={styles.infoText}>Kunstwerken laden...</Text>
        </View>
      ) : (
        <FlatList
          data={filteredPieces}
          ...
          renderItem={({ item, index }) => (
            <PieceCard
              ...
              onArtistPress={(piece) =>
                navigation.navigate("Profile", { userId: piece.user?._id })
              }
              onLocationPress={(piece) => {
                if (!piece.wall?.coordinates) return;
                const [lat, lng] = piece.wall.coordinates
                  .split(",")
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={styles.infoText}>Geen resultaten gevonden.</Text>
            </View>
          }
        />
      )}
